  ...
  {car.carStreetsListed ? (
    <>
      <Button
        ...
        size="sm"
        onClick={() => {
          const url = `${window.location.origin}/dealers/${window.location.hostname.split('.')[0] || 'carstreets'}/cars/${car.id}`
          const msg = encodeURIComponent(
            `Check out this car:\n${car.title}\nPrice: ${formatPriceSafe(car.price)}\n${url}`
          )
      ...
      <a href={`/dealers/${window.location.hostname.split('.')[0] || 'carstreets'}/cars/${car.id}`} className="flex-1">
